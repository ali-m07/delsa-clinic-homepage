"""Seed initial content. Run: python -m app.seed"""

from datetime import datetime

from sqlmodel import Session, select

from app.database import engine, init_db
from app.models import Article, Consultant, Department, Page, SiteSettings

IMG = "/static/img/services/clinic.png"
CONSULTANT_IMG = "/static/img/consultants/maryam-salehi.png"

CAREER_BODY = """
<h2>مشاوره شغلی چیست؟</h2>
<p><strong>مشاوره شغلی</strong> (راهنمایی شغلی) فرآیندی هدفمند برای کمک به افراد در شناخت توانمندی‌ها، علایق و فرصت‌های شغلی است.</p>
<h2>چه زمانی به مشاوره شغلی نیاز داریم؟</h2>
<ul>
<li><strong>انتخاب رشته و شغل اولیه</strong></li>
<li><strong>تغییر مسیر شغلی</strong></li>
<li><strong>مدیریت استرس شغلی</strong></li>
<li><strong>توسعه مهارت‌های حرفه‌ای</strong></li>
</ul>
<h2>خدمات دپارتمان مشاوره شغلی کلینیک دلسا</h2>
<ul>
<li>ارزیابی علایق، مهارت‌ها و ویژگی‌های شخصیتی</li>
<li>راهنمایی در انتخاب رشته تحصیلی و شغل مناسب</li>
<li>مشاوره برای تغییر شغل و طراحی مسیر حرفه‌ای جدید</li>
<li>کمک به مدیریت استرس و تصمیم‌گیری در شرایط اقتصادی دشوار</li>
</ul>
<blockquote><p><strong>«شغل مناسب، جایی است که توانایی شما با نیاز جامعه و علاقه درونی‌تان هم‌راستا باشد.»</strong></p></blockquote>
"""

DEPARTMENTS = [
    {
        "slug": "روان-درمانی",
        "title": "دپارتمان روان‌درمانی",
        "nav_label": "روان‌درمانی",
        "sort_order": 1,
        "intro": "اگر احساساتتان سنگین شده، با رویکرد علمی کنارتان هستیم.",
        "body_html": """
<h2>روان‌درمانی در کلینیک دلسا</h2>
<p>روان‌درمانی فرآیندی ساختارمند برای شناخت و تغییر الگوهای فکری، احساسی و رفتاری است که کیفیت زندگی را تحت تأثیر قرار می‌دهند.</p>
<h2>چه زمانی مراجعه کنیم؟</h2>
<ul>
<li>افسردگی، اضطراب یا حملات پانیک</li>
<li>استرس مزمن و فرسودگی</li>
<li>مشکلات خواب، تمرکز یا انرژی</li>
<li>بازسازی پس از بحران یا ضربه</li>
</ul>
<p>تیم روان‌درمانی دلسا با رویکردهای شناختی-رفتاری، طرحواره‌درمانی و درمان مبتنی بر ذهن‌آگاهی همراه شماست.</p>
""",
        "image_url": "/static/img/services/anxiety.png",
    },
    {
        "slug": "روانپزشکی",
        "title": "دپارتمان روان‌پزشکی",
        "nav_label": "روان‌پزشکی",
        "sort_order": 5,
        "intro": "ارزیابی و درمان تخصصی، با آرامش و دقت.",
        "body_html": """
<h2>خدمات روان‌پزشکی</h2>
<p>دپارتمان روان‌پزشکی کلینیک دلسا ارزیابی، تشخیص و درمان اختلالات روانی را با رویکردی علمی و انسانی ارائه می‌دهد.</p>
<ul>
<li>ویزیت و معاینه تخصصی</li>
<li>تجویز دارو در صورت نیاز و پیگیری درمان</li>
<li>همکاری نزدیک با تیم روان‌درمانی</li>
</ul>
""",
        "image_url": "/static/img/services/brain.png",
    },
    {
        "slug": "زوج-و-خانواده",
        "title": "دپارتمان زوج و خانواده",
        "nav_label": "زوج و خانواده",
        "sort_order": 3,
        "intro": "برای بهبود رابطه و گفت‌وگوی سالم‌تر، همراهتان هستیم.",
        "body_html": """
<h2>مشاوره زوج و خانواده</h2>
<p>روابط سالم نیازمند گفت‌وگو، شنیده شدن و یادگیری مهارت‌های جدید است. در این دپارتمان، زوجین و خانواده‌ها برای بهبود ارتباط و حل تعارض همراهی می‌شوند.</p>
<ul>
<li>مشاوره زوجین</li>
<li>مشاوره خانواده</li>
<li>مدیریت تعارض و بحران‌های رابطه</li>
</ul>
""",
        "image_url": "/static/img/services/couple.png",
    },
    {
        "slug": "کودک-و-نوجوان",
        "title": "دپارتمان کودک و نوجوان",
        "nav_label": "کودک و نوجوان",
        "sort_order": 4,
        "intro": "در مسیر رشد سالم فرزندتان، پشتیبان شما.",
        "body_html": """
<h2>مشاوره کودک و نوجوان</h2>
<p>کودکان و نوجوانان با چالش‌های منحصربه‌فردی در مسیر رشد روبه‌رو هستند. تیم تخصصی دلسا با رویکردهای متناسب با سن، به والدین و فرزندان کمک می‌کند.</p>
<ul>
<li>مشکلات رفتاری و هیجانی</li>
<li>اضطراب، افسردگی و استرس تحصیلی</li>
<li>مشاوره والدین</li>
</ul>
""",
        "image_url": "/static/img/services/child.png",
    },
    {
        "slug": "ترک-اعتیاد",
        "title": "دپارتمان ترک اعتیاد",
        "nav_label": "ترک اعتیاد",
        "sort_order": 6,
        "intro": "درمان و حمایت همراه با احترام و محرمانگی.",
        "body_html": """
<h2>ترک اعتیاد</h2>
<p>ترک اعتیاد مسیر دشواری است که با حمایت تخصصی قابل عبور است. تیم دلسا در کنار فرد و خانواده، برنامه درمانی شخصی‌سازی‌شده ارائه می‌دهد.</p>
<ul>
<li>ارزیابی و برنامه‌ریزی درمان</li>
<li>مشاوره فردی و خانوادگی</li>
<li>پیگیری و پیشگیری از عود</li>
</ul>
""",
        "image_url": "/static/img/services/freedom.png",
    },
    {
        "slug": "مشاوره-شغلی",
        "title": "دپارتمان مشاوره شغلی",
        "nav_label": "مشاوره شغلی",
        "sort_order": 7,
        "intro": (
            "دپارتمان مشاوره شغلی کلینیک دلسا، خدمات تخصصی راهنمایی شغلی را برای افرادی ارائه می‌دهد "
            "که در انتخاب، تغییر یا ادامه مسیر حرفه‌ای خود به همراهی حرفه‌ای نیاز دارند."
        ),
        "body_html": CAREER_BODY,
        "image_url": "/static/img/services/support.png",
        "meta_title": "مشاوره شغلی | کلینیک دلسا",
        "meta_description": "مشاوره شغلی در کلینیک دلسا: انتخاب شغل، تغییر مسیر حرفه‌ای و مدیریت استرس کاری.",
    },
]

CONSULTANTS = [
    {
        "slug": 'elham-mesbahi',
        "name": 'الهام مصباحی',
        "role": 'مشاور فردی، زوج‌درمانگر',
        "department_slug": 'روان-درمانی',
        "sort_order": 1,
        "image_url": '/static/img/consultants/elham-mesbahi.jpeg',
        "bio_html": """
<h2>سوابق تحصیلی و حرفه‌ای</h2>
<ul>
<li>دانش‌آموخته رشته روان‌شناسی مشاوره در مقطع کارشناسی ارشد و دکترا</li>
<li>رتبه ممتاز مقاطع کارشناسی ارشد و دکتری</li>
<li>مدرس دانشگاه‌های تهران و علوم پزشکی ایران</li>
<li>مدرس کارگاه‌های آموزشی طرحواره‌درمانی و مهارت‌های زندگی با موضوعات ارتباط مؤثر، خودآگاهی، رفتار جرات‌مندانه، مهارت حل مسئله و تصمیم‌گیری، مدیریت خشم و مدیریت استرس در مراکز خصوصی و دولتی</li>
<li>دارای پروانه اشتغال تخصصی از سازمان نظام روان‌شناسی و مشاوره ایران</li>
<li>عضو سازمان نظام روان‌شناسی و مشاوره ایران</li>
</ul>

<h2>خدمات</h2>
<ul>
<li>درمان اختلالات بالینی مانند افسردگی، اضطراب و وسواس</li>
<li>مشکلات خانوادگی، زوجی و عاطفی</li>
<li>درمانگر با رویکرد طرحواره‌درمانی در زمینه‌های مشاوره فردی، زوج‌درمانی، خانواده‌درمانی، مشاوره شکست عاطفی، فرزندپروری و درمان اختلالات روان‌شناختی از سال ۱۳۹۱</li>
</ul>
""",
    },
    {
        "slug": 'hasan-akbarzadeh',
        "name": 'حسن اکبرزاده',
        "role": 'مشاور خانواده و ازدواج',
        "department_slug": 'روان-درمانی',
        "sort_order": 2,
        "image_url": '/static/img/consultants/hasan-akbarzadeh.jpg',
        "bio_html": """
<h2>سوابق تحصیلی و حرفه‌ای</h2>
<ul>
<li>کاندیدای دکترای تخصصی مشاوره</li>
<li>کارشناس ارشد مشاوره و راهنمایی</li>
<li>دارای پروانه اشتغال تخصصی از سازمان نظام روان‌شناسی و مشاوره ایران</li>
<li>نویسنده چندین مقاله علمی پژوهشی در زمینه روان‌شناسی</li>
<li>ارائه خدمات مشاوره‌ای از سال ۱۳۸۹ در حوزه‌های فردی، عاطفی، بین‌فردی، ازدواج، طلاق، زوج، خانواده، تحصیلی، نوروفیدبک و …</li>
<li>مسئول گروه تخصصی مشاوره و روان‌شناختی دلسا</li>
<li>مشاور مرکز مشاوره دانشگاه تهران از سال ۱۳۹۱ تاکنون</li>
<li>مشاور فعال در دانشگاه‌های علوم پزشکی، تهران، هنر و آزاد</li>
<li>همکاری با مراکز مشاوره در شهر تهران</li>
<li>مدرس دوره‌های مختلف در حوزه‌های ارتباطی، عاطفی، مهارت‌های زندگی و مهارت‌های تحصیلی</li>
</ul>

<h3>دوره‌ها و کارگاه‌ها</h3>
<ul>
<li>روان‌درمانی مبتنی بر توجه‌آگاهی و شفقت</li>
<li>سایکوفارماکولوژی</li>
<li>روان‌درمانی تحلیلی کوتاه‌مدت</li>
<li>شناسایی، پیشگیری و درمان مشکلات تحصیلی دانشجویان</li>
<li>آسیب‌شناسی، پیشگیری و درمان افت تحصیلی</li>
<li>آشنایی با اختلالات شایع روان‌پزشکی</li>
</ul>

<h2>خدمات</h2>
<ul>
<li>روابط بین‌فردی و عاطفی</li>
<li>مشاوره ازدواج</li>
<li>زوج‌درمانی</li>
<li>مشاوره خانواده</li>
</ul>

<h3>رویکرد درمانی</h3>
<ul>
<li>واقعیت‌درمانی</li>
<li>CBT</li>
<li>ACT</li>
</ul>
""",
    },
    {
        "slug": 'robab-hamedi',
        "name": 'دکتر رباب حامدی',
        "role": 'زوج‌درمانی و اختلالات اضطرابی',
        "department_slug": 'زوج-و-خانواده',
        "sort_order": 3,
        "image_url": '/static/img/consultants/robab-hamedi.jpg',
        "bio_html": """
<h2>سوابق تحصیلی و حرفه‌ای</h2>
<ul>
<li>دکترای تخصصی روان‌شناسی از دانشگاه الزهرا</li>
<li>کارشناسی ارشد روان‌شناسی بالینی از دانشگاه علوم بهزیستی</li>
<li>کارشناسی روان‌شناسی بالینی از دانشگاه فردوسی مشهد</li>
<li>دارای پروانه اشتغال تخصصی از سازمان نظام روان‌شناسی و مشاوره ایران</li>
<li>عضو سازمان نظام روان‌شناسی و مشاوره ایران</li>
<li>مترجم کتاب‌های «بهبود رابطه»، «به سوی دیگران»، «درمان کمال‌گرایی با رویکرد ACT» (زیر چاپ) در انتشارات سایه سخن</li>
<li>نویسنده کتاب «کنترل خشم» در نشر قطره</li>
<li>نویسنده مقالات علمی در مورد استعاره‌ها در بیماران افسرده، نقش تمایزیافتگی در چگونگی رابطه با غیر هم‌جنس، هوش هیجانی و مهارت‌های زندگی</li>
</ul>

<h2>خدمات</h2>
<ul>
<li>درمان افسردگی و اختلالات اضطرابی مانند وسواس، فوبیا و پانیک</li>
<li>مشاوره روابط عاطفی و بین‌فردی</li>
<li>مشاوره ازدواج</li>
<li>زوج‌درمانی</li>
</ul>

<h3>رویکرد درمانی</h3>
<ul>
<li>CBT</li>
<li>CFT</li>
<li>Schema therapy</li>
</ul>

<h2>سوابق کاری</h2>
<ul>
<li>مشاوره و روان‌درمانی در مرکز مشاوره دانشگاه علوم پزشکی تهران از سال ۱۳۷۹ و کلینیک خصوصی از سال ۱۳۹۲</li>
<li>کانال عمومی «درمانگاه رابطه» در تلگرام از سال ۱۳۹۷</li>
<li>تدریس درس زوج‌درمانی برای دانشجویان کارشناسی ارشد بالینی دانشگاه تهران</li>
<li>تدریس درس روان‌شناسی عمومی برای دانشجویان رشته‌های مختلف دانشکده پیراپزشکی دانشگاه علوم پزشکی تهران</li>
<li>تدریس درس مهارت‌های زندگی برای دانشجویان رشته‌های مختلف دانشگاه علوم پزشکی تهران</li>
<li>تدریس درس روان‌شناسی رشد برای دانشجویان دانشگاه پیام نور تهران</li>
<li>داوری مقاله‌های علمی پژوهشی</li>
<li>برگزاری جلسات گروه‌درمانی برای دانشجویان دانشگاه علوم پزشکی تهران</li>
<li>برگزاری کارگاه، سخنرانی و وبینارهای متعدد برای دانشجویان و کارکنان دانشگاه تهران و علوم پزشکی تهران</li>
<li>برگزاری سخنرانی با موضوعات مختلف در حوزه بهداشت روان برای اساتید دانشکده پزشکی دانشگاه علوم پزشکی تهران</li>
<li>دبیر کمیته روان‌شناختی دانشکده پزشکی دانشگاه علوم پزشکی تهران</li>
</ul>
""",
    },
    {
        "slug": 'maryam-salehi',
        "name": 'مریم صالحی',
        "role": 'روان‌شناس سلامت',
        "department_slug": 'روان-درمانی',
        "sort_order": 4,
        "image_url": '/static/img/consultants/maryam-salehi.jpg',
        "bio_html": """
<h2>سوابق تحصیلی و حرفه‌ای</h2>
<ul>
<li>دکترای تخصصی روان‌شناسی سلامت — دانشگاه تهران</li>
<li>کارشناسی ارشد روان‌شناسی عمومی — دانشگاه الزهرا</li>
<li>کارشناسی روان‌شناسی بالینی — دانشگاه تهران</li>
<li>رتبه ۲ کنکور سراسری سال ۱۳۷۵ (سهمیه منطقه ۲)</li>
<li>دارای پروانه اشتغال تخصصی از سازمان نظام روان‌شناسی و مشاوره ایران</li>
<li>عضو سازمان نظام روان‌شناسی و مشاوره ایران</li>
</ul>

<h2>خدمات</h2>
<ul>
<li>مداخله روان‌شناختی در مشکلات سلامت روان مرتبط با بیماری‌های مزمن طبی (از جمله بیماری‌های قلبی، روماتیسم و سرطان)</li>
<li>درمان اختلالات بالینی مانند افسردگی، اضطراب و وسواس</li>
</ul>

<h3>رویکردهای درمان</h3>
<ul>
<li>شناختی-رفتاری (CBT)</li>
<li>شناختی-تحلیلی (CAT)</li>
</ul>

<h2>پروانه تخصصی اشتغال</h2>
<p>دارای پروانه تخصصی اشتغال و تایید صلاحیت حرفه ای از سازمان نظام روان شناسی و مشاوره جمهوری اسلامی ایران با مدرک دکتری به شماره ۱۲۴۰</p>

<h2>سوابق کاری — سمت‌ها و مسئولیت‌ها</h2>
<ul>
<li>مشاور گروه تخصصی مشاوره و خدمات روان‌شناختی دلسا — از ۱۳۹۸ تاکنون</li>
<li>معاون مرکز مشاوره دانشگاه تهران — از ۱۴۰۰ تا ۱۴۰۱</li>
<li>معاون برنامه‌ریزی اداره کل برنامه، بودجه و تحول سازمانی دانشگاه تهران — از ۱۳۹۴ تا ۱۳۹۹</li>
<li>سوپروایزر واحد مشاوره و روان‌درمانی مرکز مشاوره دانشگاه تهران — از ۱۳۹۳ تا ۱۳۹۴</li>
<li>کارشناس مشاور مرکز مشاوره دانشگاه تهران — از ۱۳۸۰ تا ۱۱/۷/۱۳۹۴</li>
<li>روان‌شناس تیم ورزشی دانشگاه تهران در هفتمین المپیاد فرهنگی‌ورزشی دانشجویان سراسر کشور (۱۳۸۳ — دانشگاه صنعتی اصفهان)</li>
<li>مسئول مرکز مشاوره مجتمع خوابگاه‌های فاطمیه — از ۱۳۸۳ تا ۱۳۹۳</li>
<li>مشاور و سخنران شرکت عملیات غیرصنعتی پازارگاد (ارائه خدمت به پتروشیمی‌های عسلویه) — از ۱۳۸۹ تا ۱۳۹۹</li>
<li>مشاور مراکز مشاوره خصوصی، از جمله مؤسسه مهر پروین — از ۱۳۸۹ تاکنون</li>
<li>مشاور مرکز مشاوره دانشکده علوم اقتصادی وابسته به وزارت علوم — از ۱۳۸۶ تا ۱۳۸۸</li>
<li>مسئول اجرای طرح پیشگیری اجتماع‌محور در خوابگاه‌های دانشگاه تهران — ۱۳۸۶ تا ۱۳۸۸</li>
<li>دبیر کمیته پیشگیری از آسیب‌های اجتماعی در خوابگاه‌های دانشگاه تهران — از ۱۳۸۴ تا ۱۳۸۷</li>
</ul>
""",
    },
    {
        "slug": 'fatemeh-hosseinpour',
        "name": 'فاطمه حسین\u200cپور',
        "role": 'مشاور کودک و نوجوان',
        "department_slug": 'کودک-و-نوجوان',
        "sort_order": 5,
        "image_url": '/static/img/consultants/fatemeh-hosseinpour.jpeg',
        "bio_html": """
<h2>درباره مشاور</h2>
<p>فاطمه حسین‌پور، مشاور کودک و نوجوان در کلینیک دلسا است. تمرکز او بر همراهی با کودکان، نوجوانان و والدین در مسیر رشد سالم هیجانی، رفتاری و ارتباطی است؛ در فضایی امن، محترمانه و متناسب با سن.</p>
<h2>حوزه تخصصی</h2>
<ul>
<li>مشاوره کودک و نوجوان</li>
<li>مشاوره والدین و فرزندپروری</li>
<li>مشکلات رفتاری و هیجانی</li>
<li>اضطراب، استرس تحصیلی و سازگاری</li>
<li>مهارت‌های ارتباطی و تنظیم هیجان</li>
</ul>
<h2>خدمات</h2>
<ul>
<li>جلسات مشاوره فردی کودک و نوجوان</li>
<li>جلسات مشاوره والدین</li>
<li>ارزیابی و مداخله برای مشکلات رفتاری</li>
<li>کمک به اضطراب، ترس‌ها و فشار مدرسه</li>
<li>تقویت مهارت‌های زندگی و ارتباط مؤثر در خانواده</li>
</ul>
<h2>رویکرد کاری</h2>
<p>رویکرد جلسات متناسب با سن کودک یا نوجوان طراحی می‌شود و والدین نیز در مسیر درمان و رشد، همراه نگه داشته می‌شوند. هدف، ایجاد احساس امنیت، فهمیدن نیازها و ساختن مهارت‌های پایدار برای خانه و مدرسه است.</p>
""",
    },
]

ABOUT_BODY = """
<p>صفحه درباره ما از قالب اختصاصی رندر می‌شود.</p>
"""


def seed() -> None:
    init_db()
    with Session(engine) as session:
        if session.exec(select(Department)).first():
            return

        if not session.get(SiteSettings, 1):
            session.add(SiteSettings())

        dept_map = {}
        for item in DEPARTMENTS:
            data = dict(item)
            body_html = data.pop("body_html", None)
            dept = Department(**data)
            dept.body_html = body_html or f"<p>{dept.intro}</p>"
            session.add(dept)
            session.flush()
            dept_map[dept.slug] = dept

        for raw in CONSULTANTS:
            item = dict(raw)
            dept_slug = item.pop("department_slug")
            image_url = item.pop("image_url", CONSULTANT_IMG)
            bio_html = item.pop("bio_html", "")
            person = Consultant(
                **item,
                image_url=image_url,
                bio_html=bio_html or "",
                department_id=dept_map[dept_slug].id,
            )
            session.add(person)

        session.add(Page(
            slug="درباره-ما",
            title="درباره ما",
            body_html=ABOUT_BODY,
            meta_title="درباره ما | کلینیک دلسا",
            meta_description="آشنایی با کلینیک دلسا — گروه تخصصی مشاوره و خدمات روان‌شناختی در تهران.",
        ))

        session.add(Article(
            slug="سلامت-روان-در-کار",
            title="سلامت روان در محیط کار",
            excerpt="چطور استرس شغلی را مدیریت کنیم و تعادل بین کار و زندگی برقرار کنیم.",
            body_html="<p>استرس شغلی یکی از چالش‌های رایج امروز است. شناخت علائم، مرزگذاری سالم و درخواست کمک حرفه‌ای از راهکارهای مؤثر هستند.</p>",
            published_at=datetime(2024, 6, 1),
        ))

        session.commit()
        print("Seeded site content.")
